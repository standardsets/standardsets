import { getVariations, getVariationItems, getAlternate } from './us-states';
...
describe('us-states', () => {
  it('should return all variations', () => {
    ...
    expect(variations[2].key).toEqual('abbreviationAPStyle');
    expect(variations[3].key).toEqual('abbreviationGPOStyle');
  })
  ...
  it('should return the alternate value for a given variation key', () => {
    const alternate = getAlternate('Ohio', 'postalCode');
